...
We use $z_{s, l, t}$ to denote the observed value of surveillance signal $s$ (with $s = 1$ for NHSN data, $s = 2$ for FluSurv data, and $s = 3$ for ILI+ data) in location $l$ in the week ending on date $t$.  We denote the predictive quantile at level $\alpha_k$ for the value of the signal $s$ in location $l$, generated on reference date $d$ at forecast horizon $h$ by $q_{s,l,d,h,k}$.  Note that the observation at time $t$ is the prediction target across the four combinations of reference date $d$ and forecast horizon $h$ with $d + h = t$.

In this manuscript, we evaluate forecasts using three metrics: the mean absolute error (MAE) of the predictive median, the mean weighted interval score (MWIS), and one-sided quantile coverage rates.

MAE measures the average distance between the predictive median and the eventual observation.  Denoting the predictive median for location $l$, reference date $d$, and horizon $h$ by $m_{s, l, d, h} := q_{s, l, d, h, k=12}$, the absolute error is $\vert m_{s, l, d, h} - z_{s, l, d+h} \vert$.  The mean absolute error averages the absolute error across predictions for different locations, dates, and horizons.
...
\begin{align*}
WIS(\{q_{s,l,d,h,k}: k = 1, \ldots, 23\}, z_{s,l,d+h}) &= \frac{asdf}{asdf} \sum_k QS_{\alpha_k}(q_{s,l,d,h,k}, z_{s,l,d+h}) \\
QS_{\alpha_k}(q_{s,l,d,h,k}, z_{s,l,d+h}) &= ...
\end{align*}

The quantile score for a single quantile prediction assigns an asymmetric penalty to the distance between the prediction and the observation, with the magnitudes of the penalties for underprediction and overprediction set so that in expectation, the quantile score is minimized by the quantile of the predictive distribution at a specified quantile level. The quantile score for the quantile level 0.5 is equivalent to the absolute error.

TODO formula for MAE and WIS
