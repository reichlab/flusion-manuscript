...
make_one_plot <- function(locs_to_plot) {
  ...
  loc_name_df <- target_data |> distinct(location, location_name)
  loc_name_map <- loc_name_df$location_name
  names(loc_name_map) <- loc_name_df$location
  combined_target_data_for_plot <- dplyr::bind_rows(
    ...
    dplyr::mutate(
      location_name = factor(
        location_name,
        levels = loc_name_map[locs_to_plot],
        ordered = TRUE)
    )
...
# plot for inclusion in main text
n_loc <- 6
locs_to_plot <- cum_values_by_loc$location[seq_len(n_loc)]
p <- make_one_plot(locs_to_plot)
pdf(file.path("artifacts/figures/forecasts_flusight.pdf"), width = 8, height = 6.5)
print(p)
dev.off()

# plots for inclusion in supplement:
# multiple pages, each with 9 locations
plots_per_page <- 9
n_pages <- ceiling(52 / 9)
for (page in seq_len(n_pages)) {
  locs_to_plot <- cum_values_by_loc$location[seq(from = 1 + (page - 1) * plots_per_page, by = 1, length.out = plots_per_page)]
  p <- make_one_plot(locs_to_plot)
  pdf(
    file.path(paste0("artifacts/figures/forecasts_flusight_all_models_p", page, ".pdf")),
    width = 8, height = ifelse(page < n_pages, 10.25, 8)
  )
  print(p)
  dev.off()
}
