...
for season in list(season_start_ends.keys())[1:]:
  plt.annotate(season,
               (season_start_ends[season]['start'] + pd.Timedelta(1, 'w'), 28000))
...
ax.set_ylabel('NHSN: Influenza hospital admissions')
...
fig.savefig('talks/20240220_MIDAS_flu_data_sources/plots/hhs_national.pdf',
            format = 'pdf')
#plt.show()
